...
from strategies.filesystem import (
    AgentFilesystem,
    EXECUTE_COMMAND_TOOL_NAME,
    FilesystemConfig,
)
from strategies.filesystem.config import parse_mounts
...
class AgentfsAgentStrategy(AgentStrategy):
    ...
    def _invoke(
        ...
        # --- agentfs: build the virtual filesystem from selected datasets ---
        # NOTE on credential access: this SDK version (dify_plugin==0.9.1) does not
        # expose provider-level `credentials_for_provider` anywhere on
        # `self.session` or `self.runtime` for agent strategies (verified by
        # inspecting `AgentInvokeRequest`, `AgentRuntime`, and `Session` in the
        # installed package — none carry a `knowledge_base_url`/`knowledge_api_key`
        # style credentials mapping). `self.session.credentials` is
        # `InvokeCredentials.tool_credentials: dict[provider, credential_id]`,
        # which is not usable here. The only remaining channel is `parameters`
        # itself, so we read defensively from a nested `credentials` dict if
        # present, falling back to flat keys on `parameters`.
        raw_credentials = parameters.get("credentials")
        credentials: dict[str, Any] = (
            raw_credentials if isinstance(raw_credentials, dict) else {}
        )
        base_url = credentials.get("knowledge_base_url") or parameters.get(
            "knowledge_base_url", ""
        )
        api_key = credentials.get("knowledge_api_key") or parameters.get(
            "knowledge_api_key", ""
        )
        agent_fs: AgentFilesystem | None = None
        try:
            mounts = parse_mounts(fc_params.datasets)
        ...
        if mounts:
            try:
                fs_config = FilesystemConfig(
                    mounts=mounts,
                    truncate_kb=fc_params.truncate_kb,
                    expose_semantic_search=fc_params.expose_semantic_search,
                    include_workspace_tree=fc_params.include_workspace_tree,
                )
                agent_fs = AgentFilesystem.build(fs_config, base_url, api_key)
            except Exception as exc:  # noqa: BLE001
                yield self.create_text_message(
                    f"Failed to mount datasets as filesystem: {exc}"
                )
                return
        ...
        try:
            ...
            self.query = query
            base_instruction = fc_params.instruction or ""
            if agent_fs is not None:
                fs_instructions = agent_fs.instructions()
                self.instruction = (
                    f"{base_instruction}\n\n{fs_instructions}".strip()
                    if fs_instructions
            ...
            tools = list(fc_params.tools or [])
            if agent_fs is not None:
                tools.append(agent_fs.tool_entity())
            ...
            while function_call_state and iteration_step <= max_iteration_steps:
                ...
                if tool_calls and iteration_step == max_iteration_steps and max_iteration_steps > 1:
                    ...
                    for tool_call_id, tool_call_name, tool_call_args in tool_calls:
                        ...
                        if (
                            agent_fs is not None
                            and tool_call_name == EXECUTE_COMMAND_TOOL_NAME
                        ):
                            command = tool_call_args.get("command", "")
                            fs_result = agent_fs.execute(command)
                            tool_response = {
                                "tool_call_id": tool_call_id,
                                "tool_call_name": tool_call_name,
                                "tool_call_input": tool_call_args,
                                "tool_response": json.dumps(
                                    fs_result, ensure_ascii=False
                                ),
                            }
                            tool_responses.append(tool_response)
                            current_thoughts.append(
                ...
                # update prompt tool (skip the synthetic filesystem tool)
